...
from sqlalchemy import Integer, ForeignKey, String, Text, DateTime, func, Enum
...
class MfsAuditLog(Base):
    ...
    user_id: Mapped[int] = mapped_column(Integer, ForeignKey(f"{settings.POSTGRES_SCHEMA}.user.id"), nullable=False)
    username:Mapped[str] = mapped_column(String(64), nullable=False)
    action: Mapped[Action] = mapped_column(Enum(
        Action,
