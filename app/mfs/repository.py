...
class MfsAuditLogRepository:
    ...
    async def get_by_id(self, mfs_id: int) -> MfsAuditLog:
        mfs_audit_log = await self.db.execute(
            select(MfsAuditLog)
            .where(MfsAuditLog.id == mfs_id)
        )
    ...
    async def get_all_by_user(self, user_id: int, offset: int = 0, limit: int = 200) -> list[MfsAuditLog]:
        mfs_audit_logs = await self.db.execute(
            select(MfsAuditLog)
