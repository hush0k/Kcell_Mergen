...
from app.vacation_schedule.schemas import VacationScheduleResponse, VacationScheduleCreate, VacationScheduleUpdate, \
...
def get_vacation_schedule_service(db: Annotated[AsyncSession, Depends(get_db)]) -> VacationScheduleService:
...
@router.post("/", response_model=VacationScheduleResponse, status_code=http_status.HTTP_201_CREATED)
...
async def delete_vacation_schedule(
    ...
    return await service.delete(vacation_id, current_user)

@router.get("/vacation-remaining-days", response_model=list[VacationScheduleRemainingList])
