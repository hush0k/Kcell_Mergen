...
class Control(Base, TimeStampMixin):
    ...
    frequency:Mapped[Frequency] = mapped_column(Enum(Frequency), default=Frequency.DAILY, nullable=False)
    deadline_at: Mapped[datetime] = mapped_column(DateTime, nullable=True)
    responsible_id: Mapped[int] = mapped_column(ForeignKey(f"{settings.POSTGRES_SCHEMA}.user.id"))
    backup_id: Mapped[int] = mapped_column(ForeignKey(f"{settings.POSTGRES_SCHEMA}.user.id"))
    ...
    status: Mapped[ControlStatus] = mapped_column(Enum(ControlStatus), default=ControlStatus.ACTIVE)
