    ...
    const handleLogin = async () => {
        ...
        try {
            ...
            if (err instanceof ApiError && err.status === 401) {
                setError({ username: " ", password: "Неверный логин или пароль" })
            } else if (err instanceof ApiError && err.status === 400) {
                setError({ username: " ", password: "Поля не до конца заполнены" })
            } else {
    ...
    return (
        <div className="inline-flex flex-row shadow-mg-login bg-mg-bg">
            ...
            <div className="flex flex-col justify-between px-12 py-14 relative overflow-hidden bg-mg-purple w-[32rem] h-[34rem] rounded-l-login">
                ...
                <div style={{ position: "absolute", width: "340px", height: "340px", borderRadius: "50%", right: "-120px", top: "-110px", background: "radial-gradient(circle at 30% 30%, rgba(168, 216, 95, 0.55), rgba(168, 216, 95, 0) 70%)" }} />
                <div style={{ position: "absolute", width: "230px", height: "230px", borderRadius: "50%", left: "-90px", bottom: "-70px", background: "radial-gradient(circle, rgba(255, 255, 255, 0.18), rgba(255, 255, 255, 0) 70%)" }} />
            </div>
            ...
            <div className="bg-mg-surface w-[32rem] h-[34rem] rounded-r-login px-12 py-24">
                ...
                <form
                    ...
                    className="flex flex-col space-y-5 text-xs font-semibold text-mg-text-2 mt-7"
                >
                    {/* Username */}
                    ...
                    <div className="flex flex-col space-y-2">
                        ...
                        <div className="relative w-full">
                            ...
                            >
                                {show
                                    ? <Eye size={20} strokeWidth={2.2} color="var(--mg-text-3)" />
                                    : <EyeClosed size={20} strokeWidth={2.2} color="var(--mg-text-3)" />
                                }
                            </button>
                        </div>
                    </div>
