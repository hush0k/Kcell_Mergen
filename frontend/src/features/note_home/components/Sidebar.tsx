...
export function Sidebar() {
    ...
    return (
        <div className="flex flex-col h-full w-[20rem] border-r border-nt-outline-variant bg-nt-surface-container-low">
            {/* Верхний фиксированный блок: аватар, навигация, кнопка "Новая папка" */}
            <div className="flex flex-col items-center w-full shrink-0 bg-nt-surface-container-low">
                <div className="flex flex-row w-full space-x-2 px-4 py-2 border-b border-nt-outline-variant">
                ...
                <Button
                    icon={<LuPlus />}
                    text={"Новая Папка"}
                    className="w-auto rounded-none bg-nt-secondary-fixed-dim px-14 py-2 mt-6"
                    onClick={() => setIsNewFolderOpen(true)}
                />
            </div>

            {/* Средний блок: список папок. Растягивается на оставшееся место, скроллится только при переполнении */}
            <div className="flex-1 min-h-0 overflow-y-auto flex flex-col space-y-1 w-full pl-5 mt-6 [mask-image:linear-gradient(to_bottom,black_90%,transparent_100%)]">
            ...
            {/* Нижний фиксированный блок: кнопка Mergen */}
            <div className="flex flex-row items-center justify-between w-full px-5 py-5 shrink-0 bg-nt-surface-container-low">
                <Button
